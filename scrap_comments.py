...
def main(argv):
    ...
    parser.add_argument(
        "--url", "-u", help="Youtube url of the video",
    )
    ...
    try:
        ...
        _, youtube_id = url.split("v=")

        if output:
            with open(file=output, mode="w") as f:
                comments = [
                    comment for comment in download_comments(youtube_id=youtube_id)
                ]
                f.write(json.dumps(comments))
        else:
            for comment in download_comments(youtube_id=youtube_id):
                print(comment)
